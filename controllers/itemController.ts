...
export const createItem = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (parsed.is_tax_inherit){
    if (!catId && !subId) {
      throw new BadRequestError("category or sub-category missing how tax inherit then?");
    }
  }
  ...
  if (!parsed.is_tax_inherit) {
    if (data.tax_applicable && data.tax_percentage <= 0) {
      throw new BadRequestError('tax_percentage must be > 0 when tax_applicable is true');
    }
    if (!data.tax_applicable && data.tax_percentage > 0) {
      throw new BadRequestError('tax_percentage must be 0 when tax_applicable is false');
    }
  }
  ...
  if (catId) {
    ...
    if (!exists) throw new NotFoundError('Category', catId);
    data.category = { connect: { id: catId } };
  } else if (subId) {
    const exists = await prisma.subcategory.findUnique({ where: { id: subId } });
    if (!exists) throw new NotFoundError('Subcategory', subId);
  ...
  if (parsed.type_of_pricing) {
    ...
    if (typeKey === 'A') {
      ...
      if (parsed.base_price !== undefined && parsed.base_price > 0) {
        throw new BadRequestError('base_price cannot be provided for COMPLIMENTARY pricing (type C)');
      }
      data.type_of_pricing = typeKey;
      ...
      if (!Object.prototype.hasOwnProperty.call(req.body, 'base_price')) {
        throw new BadRequestError('base_price is required for DISCOUNTED pricing (type D)');
      }
      if ((parsed.base_price ?? 0) <= 0) {
        throw new BadRequestError('base_price must be > 0 for DISCOUNTED pricing (type D)');
      }
      try {
        ...
        throw new BadRequestError(`Invalid price_config: ${e.message}`);
      }
    } else {
      try {
        const norm = require('../services/price_config').normalizePriceConfig(typeKey, parsed.price_config, parsed.base_price);
    ...
    throw new BadRequestError('price_config provided but type_of_pricing is missing');
  }
  ...
  if (duplicate) throw new ConflictError('An item with this name already exists under this parent');
  ...
  if (data.type_of_pricing === 'E') {
    if (!data.avl_times || data.avl_times.length === 0) {
      throw new BadRequestError("DYNAMIC pricing (type E) requires avl_times to be provided and non-empty");
    }
    ...
    if (!data.price_config || !data.price_config.config || !Array.isArray(data.price_config.config.windows) || data.price_config.config.windows.length === 0) {
      throw new BadRequestError('DYNAMIC pricing requires a non-empty "windows" array in price_config');
    }
    ...
    if (!allContained) throw new BadRequestError("Each DYNAMIC pricing window must be contained within one of the item's avl_times");
  }
...
export const getItemsByParent = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if ((!categoryId && !subcategoryId) || (categoryId && subcategoryId)) {
    throw new BadRequestError('Provide exactly one of categoryId or subcategoryId');
  }
...
export const getItemPrice = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (missingMandatory.length > 0) {
    throw new BadRequestError('Missing mandatory addons');
  }
  ...
  if (invalidAddons.length > 0) {
    throw new BadRequestError('Invalid addon IDs provided');
  }
...
export const patchItem = async (req: Request, res: Response, next: NextFunction) => {
  ...
  const catId = parsed.categoryId?.trim();
  const subId = parsed.subcategoryId?.trim();
  if (catId && subId) throw new BadRequestError('Item may belong to either a category or subcategory, not both');
  ...
  if (subId && subId !== existing.subcategoryId) {
    ...
    if (!sub) throw new NotFoundError('Subcategory', subId);
  }
  ...
  if (parsed.name && parsed.name !== existing.name) {
    ...
    if (dup) throw new ConflictError('An item with this name already exists under this parent');
  }
  ...
  if (newIsInherit === true && hasTaxPayload) {
    throw new BadRequestError('Invalid request: Cannot accept tax payloads when is_tax_inherit is true.');
  }
  ...
  if (newIsInherit === false) {
    ...
    if (resultingTaxApp === null || resultingTaxApp === undefined || resultingTaxPct === null) {
      throw new BadRequestError('Explicit tax payload required when disabling inheritance');
    }
    if (resultingTaxApp === true && (resultingTaxPct === null || resultingTaxPct === undefined || resultingTaxPct <= 0)) {
      throw new BadRequestError('If tax_applicable is true, tax_percentage must be > 0');
    }
  ...
  if (targetType === 'D') {
    ...
    if (resultingBase === null || resultingBase === undefined || resultingBase <= 0) {
      throw new BadRequestError('base_price must be provided and > 0 for DISCOUNTED pricing (type D)');
    }
  }
  ...
  if (targetType === 'E') {
    ...
    if (!effectiveAvl || !Array.isArray(effectiveAvl) || effectiveAvl.length === 0) {
      throw new BadRequestError('DYNAMIC pricing (type E) requires avl_times to be present (either in request or already set on the item).');
    }
    ...
    if (!allContained) throw new BadRequestError("Each DYNAMIC pricing window must be contained within one of the item's avl_times for this patch.");
  }
  ...
  await prisma.$transaction(async (tx) => {
    for (const t of types) {
      ...
      if (typeof configPayload === 'undefined') {
        throw new BadRequestError(`Missing price_config for type '${t}'`);
      }
      ...
      if (t === 'D') {
        ...
        if (invalidCount > 0) {
          throw new BadRequestError(`There are ${invalidCount} item(s) with missing or non-positive base_price for DISCOUNTED pricing (type D). Ensure base_price > 0 before applying discounted configs.`);
        }
      }
